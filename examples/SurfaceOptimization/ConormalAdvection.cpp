...
int main(int, char**)
{
  ...
  for (int i = 0; i < 100; i++)
  {
    ...
    MMG::AdvectS(mmgDist, mmgCo).step(0.01);

    auto mmgImplicit = MMG::ImplicitDomainMesherS().discretize(mmgDist);
    mmgImplicit.save("out/mmg." + std::to_string(i) + ".mesh");

    Omega = Cast(mmgImplicit).to<Mesh<Traits::Serial>>();

    // mmgDist.save("mmg.sol");
    // mmgMesh.save("mmg.mesh");
  }
