...
// Parameters
static constexpr Geometry::Attribute Clamp = 3; // u = 0
static constexpr Geometry::Attribute Gamma = 4; // dn u = 0
static constexpr Geometry::Attribute GammaN = 5; // dn u = f

static constexpr Geometry::Attribute dClamp = 5;
static constexpr Geometry::Attribute dGammaN = 6;
...
//     dOmega.trace({{{ Clamp, Gamma }, dClamp }, {{ GammaN, Gamma }, dClamp }});
...
//     auto dist = MMG::Distancer(dsfes).setInteriorDomain(Clamp)
//                                      .distance(dOmega);
// 
//     Solver::CG cg;
// 
...
//           + FaceIntegral(he * u, v).over(Clamp)
//           - Integral(f, v);
//     state.solve(cg);
// 
...
//             + BoundaryIntegral(he * p, q).over(Clamp)
//             - Integral(dj * q);
//     adjoint.solve(cg);
// 
...
//     {
//       return Integral(u).compute() + ell * mesh.getPerimeter(Clamp);
//     };
...
//                 u.getSolution() * p.getSolution(), Dot(conormal, w)).over(dClamp)
//             + ell * FaceIntegral(conormal, w).over(dGammaN)
