...
/**
 * @example Transient 2D incompressible Oseen flow with Taylor–Hood elements and PETSc
 *
 ...
 * Output:
 *   - OseenFlow.mesh
 *   - OseenFlow_velocity_XXXXXX.gf
 *   - OseenFlow_pressure_XXXXXX.gf
 *   - flux.txt
 *   - pressure.txt
 *
 * Suggested command:
 * VECLIB_MAXIMUM_THREADS=1 OPENBLAS_NUM_THREADS=8 OMP_NUM_THREADS=8 \
 * ./examples/PETSc/PDEs/PETSc_Seq_OseenFlow \
...
int main(int argc, char** argv)
{
  ...
  Alert::Info() << "Saving MEDIT mesh to OseenFlow.mesh..." << Alert::Raise;
  mesh.save("OseenFlow.mesh", IO::FileFormat::MEDIT);

  Alert::Info() << "Setting up OseenFlow.xdmf ..." << Alert::Raise;

  IO::XDMF xdmf("OseenFlow");
  xdmf.setMesh(mesh);
