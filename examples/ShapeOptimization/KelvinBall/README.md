...
`KelvinBallMetrics` is the validation executable. It takes a conforming
one-twenty-fourth chamber mesh, computes the resistance coefficients with a
stabilized P1--P1 Stokes discretization and rotational Nitsche coupling, then
reconstructs the complete fluid mesh from the 24 proper cubic rotations and
solves the full-domain problems independently. The chamber and full-domain
values of
...
is written to `KelvinBallMetrics.xdmf`.

```sh
KelvinBallMetrics chamber.mesh 0.12
```

The optional third and fourth numerical arguments are the dimensionless
Nitsche penalty and the coefficient of the pressure stabilization
`h^2 / mu`. Their defaults are 320 and 0.05.
...
`KelvinBall` contains the optimization loop. It calls the same chamber metric
evaluator used by `KelvinBallMetrics`, constructs the Hadamard density, projects
...
so its negative region is the spherical part of the chamber. The resolution
is specified either by the number of grid points on an edge or by a requested
mesh size. In the latter case, the resolution is rounded up and the effective
mesh size is `2 / (n - 1)`.
...
arguments, the executable uses `--n=13`, `--iterations=1`, `--penalty=320`,
`--stabilization=0.05`, `--regularization=4`, and `--step=0.1`.
