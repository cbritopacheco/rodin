...
struct Config {
  ...
  // strong Dirichlet fluid wall the domain fills correctly from step 1, so the
  // ramp's only job now is to ease the wall load; a longer window (~100 steps)
  // keeps the loose coupling well-behaved.
  Real pressureRampTime = 0.025;
  ...
  // Robin parameter for the SOLID-side FSI coupling only.  The fluid side now
  // uses a strong Dirichlet wall (u = u_s), so this no longer appears in the
  // fluid problem.
  Real robinAlpha = 2e4;
};
...
//      VectorFunctions (interfaceSolidVelocity, interfaceFluidVelocity,
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    auto fluidStress = VectorFunction(dim, [&](const Point &xs) {
      ...
      Math::SpatialVector<Real> value(dim);
      value.setZero();
      const auto force = tractionFSI(xf);
      for (Index i = 0; i < static_cast<Index>(dim); ++i)
        value(i) = force(i);

      // Areal stretch J_s = A_t / A_0 of the solid FSI face: pulls the
      // current-config traction back to the solid reference configuration.
      Real stretch = 1.0;
      ...
      if (dim == 3 && verts.size() == 3) {
        ...
        for (Index c = 0; c < 3; ++c) {
          x0(c) += dOld[dh.getGlobalIndex({0, verts[0]}, c)];
          x1(c) += dOld[dh.getGlobalIndex({0, verts[1]}, c)];
          x2(c) += dOld[dh.getGlobalIndex({0, verts[2]}, c)];
        }
    ...
    flow =
        ...
        0.5 * cfg.fluidDensity * Integral(divTransportLag * Dot(u, v)) +
        ...
        // Dirichlet-Neumann FSI transmission (fluid side): STRONG kinematic
        // condition u = u_s on the moving wall, with the just-computed solid
        // interface velocity sampled cross-mesh.  This replaces the earlier
        // weak Robin transmission (alpha (u - u_s) = t_f), which at any
        // practical alpha is dominated by the bulk rho/dt stiffness and lets
        // the self-traction term cancel the wall stress -- the interface then
        // behaves nearly traction-free (an OPEN boundary), the pressure cannot
        // be sustained and the domain never fills.  With the strong Dirichlet
        // wall the fluid is properly confined (at step 1, u_s = 0, this is
        // exactly the reference CoupledLV0DCoronary3D rigid wall, so the
        // pressure-driven field develops from the very first solve).  The
        // dynamic half of the coupling is unchanged: the solid receives the
        // fluid traction 'fluidStress' as its Neumann load
    ...
    // Fluid interface velocity u_f^n evaluated at a SOLID interface point.
    ...
    solid =
        ...
        // Robin transmission on the FSI interface (solid side):
        //   sigma_s n_s + alpha d(d_s)/dt = alpha u_f^{n-1} + t_f^{n-1},
        // with the Newmark interface velocity
        //   d(d_s)/dt = vPred + solidVelocityCoeff (dState - dPred),
        // so  alpha d(d_s)/dt -> robinVelocityCoeff (d, dState) - robinVelocityCoeff dPred
        //                        + alpha vPred,  and the fluid traction 'fluidStress'
        // (pulled back to the solid reference config) is the Neumann RHS.
        + robinVelocityCoeff * BoundaryIntegral(d, w).over(BoundarySolid::FSI) +
        robinVelocityCoeff *
            BoundaryIntegral(dState, w).over(BoundarySolid::FSI) -
        robinVelocityCoeff *
            BoundaryIntegral(dPred, w).over(BoundarySolid::FSI) +
        cfg.robinAlpha * BoundaryIntegral(vPred, w).over(BoundarySolid::FSI) -
        cfg.robinAlpha * BoundaryIntegral(interfaceFluidVelocity, w)
    ...
    for (size_t step = 1; step <= cfg.nsteps; ++step) {
...

      {
        for (size_t couple = 1; couple <= cfg.couplingIterations; ++couple) {
          ...
          // Solid velocity / acceleration consistent with the new state.
          ...
          // previous mesh Omega^n.  aleDisp/aleDispOld live on the velocity FES
          // 'uh', so the vertex displacements MUST be indexed with 'uh'
          ...
          // assemble all Omega^{n+1} fluid terms there.
          moveMeshWithVertexDisplacement(meshFluid, referenceVertices, uh,
                                         aleDisp);
