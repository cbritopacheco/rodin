 ...
 */
/**
 * @file LeftAtrium2D.cpp
 * @brief Driver for the 2D left-atrium flow and coagulation-kinetics example.
 *
 * Run (from the build directory):
 *   python3 ../examples/Heart/LeftAtrium2D/make_la2d_mesh.py \
 *           ../resources/examples/Heart/LeftAtrium2D/RectLAA.mesh \
...
namespace Rodin::Examples::Heart
{
  ...
  void LeftAtrium2D::setupWallShear()
  {
    ...
    const auto traction =
      VectorFunction(mu * Dot(m_symRec0, normal), mu * Dot(m_symRec1, normal));
  ...
  void LeftAtrium2D::writeCSVRow(int cycle)
  {
    ...
    for (const auto q : m_qInPatch)
      m_csv << q << ',';
    m_csv << m_outletPressure << ',' << m_speed << ',' << m_velocityScale << ',' << tawss
    ...
    for (int step = 0; step < totalSteps; ++step)
    {
      ...
      if (isRoot() && (m_step < 3 || step % 20 == 0 || endOfCycle))
      {
        ...
        Alert::Info() << "[2D] max|u|=" << m_speed << " m/s ("
                      << (m_speed / m_velocityScale) << "x scale)  p_pv=" << m_pIn
...
int main(int argc, char** argv)
{
  ...
  try
  {
...

    {
      ...
      got = PETSC_FALSE;
      PetscOptionsGetReal(PETSC_NULLPTR, PETSC_NULLPTR, "-la2d_pspg_scale", &real, &got);
      if (got)
        cfg.pspgScale = real;

      got = PETSC_FALSE;
      PetscOptionsGetReal(
        PETSC_NULLPTR, PETSC_NULLPTR, "-la2d_pspg_residual", &real, &got);
      if (got)
