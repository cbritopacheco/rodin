...
struct Config {
  ...
  // Pressure-gradient (PSPG / grad-p) stabilization scale.  REQUIRED for the
  // equal-order P1/P1 pair (no inf-sup stability): adds the continuity term
  //   + int tau_p (grad p . grad q),  tau_p = pgpScale * tau1 / rho,
  // which is the classical Brezzi-Pitkaranta / Hughes-Franca pressure
  // stabilization (tau1 evaluated at the lagged velocity -> tau is a
  // coefficient, never an unknown).  Set to 0 ONLY for testing; the pressure
  // will then be unstable on P1/P1.
  Real pgpScale = 1.0;
