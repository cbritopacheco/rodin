 ...
 */
/**
 * Coupled 0D left ventricle / 3D coronary flow solver.
 *
 * Fluid: rigid-wall generalized-Newtonian Navier-Stokes on the epicardial
 * tree, P2/P1, BDF1, assembled either as a full Newton system (PETSc SNES) or
 * as an Oseen/Picard system with lagged transport coefficients (PETSc KSP).
 *
 * Heart: the CCMLC2014 0D left ventricle supplies the inlet pressure p_ar and
 * the cavity pressure p_LV; the intramyocardial pressure is p_im = alpha p_LV.
 *
 * Outlet i, a Starling resistor in an intramyocardial bed:
 *
 *   3D --[R_a Phi_a, assembled implicitly]--> (p_c, C) --[R_v Phi_v]--> P_RA
 *                                                 ^
 *                                              p_im(t)
 *
 * with the single state p_tm = p_c - p_im,
 *
 *   C dp_tm/dt = Q - q_v,   q_v = [p_c - max(p_im, P_RA)] / (R_v Phi_v),
 *   p_out      = p_im + p_tm      (+ R_a Phi_a Q, assembled in the 3D form),
 *
 * and a throat that shuts for p_tm <= 0 (Permutt-Riley; Downey & Kirk 1975).
 * Phi_a and Phi_v are the generalized-Newtonian resistance multipliers of the
 * WRMS closure, normalized by the Newtonian calibration viscosity mu_N.
 *
 * The arteriolar resistance enters the 3D form as R_a Phi_a A (u.n)(v.n)
 * instead of being lagged, so the 3D-0D exchange is stable for any dt.
 *
 * The triplet (R_a, R_v, C) of every outlet is calibrated at startup from the
 * outlet areas measured on the mesh, a Murray-law flow split and the resting
 * pressure budget. See RCR_formulacion_minima.tex.
 */
...
namespace Rodin::Examples::Heart
{
  class CoupledLV0DCoronary3D
  {
    public:
       ...
       *
       * @details The single state is the microvascular transmural pressure
       *          p_tm = p_c - p_im. The compartment is intramyocardial, so
       *          p_im is the reference of the compartment and not a source
       *          added to the balance, and the venous limb is a Starling
       *          resistor whose throat closes at p_im (Permutt-Riley;
       *          Downey & Kirk 1975). p_tm is then bounded below by zero and
       *          above by R_v max(q_a), with time constant C R_v.
       *
       *          See RCR_formulacion_minima.tex.
       */
      struct RCR
      {
          /// @brief Microvascular transmural pressure, p_tm = p_c - p_im. State.
          Real ptm = 0.0;
          ...
          Real pim = 0.0;
          /// @brief Arteriolar lumped resistance at the reference viscosity.
          ///        Assembled implicitly on the outlet boundary.
          Real Ra = 0.0;
       ...
       *
       *          The haematocrit phi and the plasma viscosity mu_F set the
       *          high-shear level, where the coronary bed operates; k_0 sets
       *          the low-shear aggregation rise. The two axes are therefore
       *          independent, which is what the parametric studies require.
       */
      struct Quemada
      {
          ...
          /// @details k_0, k_inf and gamma_c are functions of the haematocrit,
          ///          not constants: the law has a packing limit
          ///          phi_max = 2/k, so a k_0 frozen at its phi = 0.45 value
          ///          diverges above phi = 0.46. Left non-positive, they are
          ///          evaluated from the Cokelet correlations:
          ///
      ...
      /**
       * @brief Tabulated WRMS closure and scalar-solve tolerances of the 0D
       *        outlet.
       *
       * @details Writing the flow of a generalized-Newtonian fluid in a tube
       *          as Hagen-Poiseuille with an apparent viscosity,
       *
       ...
       *
       *          so mu_ap is a universal function of the wall shear stress for
       *          a given rheology: it depends neither on R, nor on L, nor on
       *          the branch. It is tabulated once in the nominal shear rate
       *          gammadot = 4Q/(pi R^3) = tau_w / mu_ap and the 0D element
       *          interpolates. With 241 log-spaced nodes the maximum log-log
       *          interpolation error is 0.09 per cent.
       */
      struct OutletFlowLaw
      {
          ...
          /// @brief Residual venous conductance kept when the throat is shut,
          ///        as a fraction of the open value.
          /// @details A hard on/off switch leaves the Newton Jacobian without
          ///          a restoring term as soon as p_tm crosses zero, so the
          ///          outlet becomes a pure integrator of the 3D flux. The
          ///          leak keeps J away from C/dt and the switch
          ///          differentiable. At 1e-3 it moves the open-lumen solution
          ///          by less than 0.1 per cent.
          Real closedLumenLeak = 1.0e-3;
          /// @brief Enforce the lower bound p_tm >= 0 assumed by the closure.
          /// @details The bound holds exactly while the venous lumen is open;
          ///          imposing it makes it hold unconditionally.
       ...
       *          gammadot = 4Q/(pi R^3), which is monotone in tau_w, so the
       *          lookup required by the 0D element (given a flow, return the
       *          apparent viscosity) needs no root finding.
       */
      struct WRMSTable
      {
          ...
          /// @brief Apparent viscosity at a nominal shear rate. Clamped rather
          ///        than extrapolated: outside the table the plateaus of the
          ///        constitutive law are the correct continuation.
      ...
      struct Config
      {
          ...
          /// @brief Newtonian reference viscosity mu_N of the outlet
          ///        calibration. The rheological multipliers Phi are
          ///        normalized by it and never by the running rheology, so a
          ///        change of blood properties moves the flow instead of being
          ...
          /// @details p_c is the microvascular pressure and initializes near
          ///          the right atrial level. The calibration overwrites these
          ///          with the steady state of the calibrated network.
          RCR defaultRCR{.ptm = 1400.0, .pc = 2000.0, .pout = 2000.0};

          /// @brief Enable the anatomical outlet calibration at startup.
          /// @details Outlet areas are measured on the mesh and the branch
          ///          flows split as Q_i proportional to r_i^3 (Murray's law)
          ///          summing to lcaTargetFlow. Disabling it leaves a single
          ///          vessel per bed and the configured geometry, for
          ///          diagnostic use only.
          ...
          ///          Q_i = lcaTargetFlow * w: the resting flow is an input
          ///          and the resistance follows, so nothing upstream, a
          ///          stenosis included, can move the calibration. When true
          ///          the arteriolar multiplicity N_a is prescribed instead
          ///          (arteriolarCount, split by the same Murray weight) and
          ///          the flow becomes a prediction, Q_i = pi r_a^2 v_a N_a.
          ///          The two closures coincide for
          ///          arteriolarCount = lcaTargetFlow / (pi r_a^2 v_a), which
          ...
          ///          adenosine hyperaemia under which FFR is measured. At
          ///          rest the microvascular bed carries almost all the
          ///          resistance and an epicardial stenosis below ~90 per cent
          ///          area is not flow limiting; a stenosis study therefore
          ///          requires this above one.
          Real vasodilationFactor = 1.0;

          /// @brief Total microvascular compliance (m^3/Pa), split across the
          ///        outlets by the Murray weight.
          /// @details With R_v it sets the run-off time constant
          ///          tau = C R_v, reported at calibration and to be checked
          ///          against the diastolic decay. The systolic/diastolic
          ///          inflow ratio moves only from 0.38 to 0.30 over
          ///          1e-10 to 3e-9, because the systolic impediment comes
          ///          from the tissue pressure entering p_out and not from the
          ///          storage.
          ...
          ///          venular. It is the only quantity splitting the budget:
          ///          R_v = f_v dP / Q and R_a = (1 - f_v) dP / Q.
          Real venularPressureFraction = 0.13;

          /// @brief Fraction of LV pressure transmitted to the
          ///        intramyocardial compartment, p_im = alpha p_LV.
          /// @details Bed-averaged transmission. The tissue pressure runs from
          ///          about p_LV in the subendocardium to a small fraction of
          ///          it in the subepicardium, so a volume-weighted average
          ///          over the wall is of order 0.5-0.7. It is the only
          ///          constant of the tissue-pressure block, and the model is
          ///          sensitive to it: the systolic/diastolic inflow ratio
          ///          should be checked against the chosen value.
          ...
          /// @details R_mu is replaced by R_inf = 8 mu_inf L / (N pi R^4): the
          ///          WRMS table is built with mu(gammadot) == mu_inf, so
          ///          Phi = mu_inf/mu_N is constant and the pressure-flow
          ///          relation of every outlet is linear. The plateau is taken
          ///          from the active law (mu_inf for Carreau-Yasuda,
          ///          mu_F (1 - k_inf phi/2)^-2 for Quemada), so the flag is
          ///          exactly "same mu_inf, no shear thinning".
          ///
          ///          Only the 0D closure changes; Config::viscosity still
          ///          drives the 3D field, so a run and its R_mu twin differ
          ///          in the reduced outlet model alone.
          bool constantOutletResistance = false;

          /// @brief Inlet tangential velocity damping coefficient.
    ...
    private:
      struct StepData
      {
          ...
          /// @brief Mechanism diagnostics, summed or averaged over the
          ///        outlets: the state p_tm, the stored volume C p_tm and the
          ///        rheological modulation of each limb.
          Real ptm = 0.0;
          Real muARatio = 1.0;
          Real muVRatio = 1.0;
      ...
      /// @details Implicit Euler with a scalar Newton on the transmural
      ///          pressure p_tm. The residual derivative is
      ///          C/dt + dq_v/dp_tm > 0 everywhere, so the iteration converges
      ///          from any iterate without line search or bracketing.
      ...
      /// @brief Recovered wall shear stress field (P2 velocity space),
      ///        written to XDMF as "shearStress".
      ...
      // Wall-shear recovery projections. Held as members so that their mass
      // matrix and preconditioner are assembled once and reused by every call
      // to computeWallShear().
