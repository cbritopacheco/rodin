...
\cbp{
The second application considers the coupled LV-0D/coronary-3D model of
figure~\ref{fig:coronary-setup}. The computational domain $\Omega$ is a
three-dimensional coronary artery with one inlet, one rigid arterial wall on
which no-slip is imposed, and six terminal outlets $\Gamma_\mu^{(i)}$,
$i=1,\ldots,6$. The flow in $\Omega$ is governed by the incompressible
Navier--Stokes equations \eqref{eq:fluid-general-a}--\eqref{eq:fluid-general-b}
with the Carreau--Yasuda apparent viscosity \eqref{eq:carreau-yasuda}. Each
terminal outlet carries the single-state $\mathcal{R}_{\mu}$ model of
\S\ref{sec:coronary-outlet-model}, which is where the closure of
\S\ref{sec:Rmu} is exercised.

The coupling to the driver of \S\ref{sec:ApplicationI} is one-way and uses its
state at two locations: the arterial pressure $p_{\rm ar}(t)$ is imposed as
the inlet traction of $\Omega$, and the ventricular pressure $p_{\rm LV}(t)$
sets the intramyocardial pressure acting on the distal coronary compartments.
The reduced circulation follows the pathway
\begin{equation}
p_{\rm LA}(t)
...
\end{equation}
where $p_{\rm LA}$ denotes the prescribed left-atrial pressure, $p_{\rm LV}$
...
pressure, while each terminal outlet appends the coronary branch
\begin{equation}
p_{\rm ar}
\longrightarrow
\Omega
...
\end{equation}
The systemic distal pressure $p_d$ and the coronary microvascular pressures
$p_c^{(i)}$ are therefore distinct variables. The discretisation and the
solution algorithm are standard and deferred to \S\ref{sec:coronary-numerics};
we describe the outlet model first, since it is where the closure of
\S\ref{sec:Rmu} enters the problem.
}
...
\subsubsection{Discretisation and solution algorithm}
\label{sec:coronary-numerics}

\cbp{
The mesh of $\Omega$ contains 35k tetrahedral elements, 11k vertices and 19k
boundary faces, and is partitioned over 30 MPI ranks. The spatial
discretisation uses Taylor--Hood finite elements~\citep{TaylorHood1973},
$\boldsymbol{u}_h\in[\mathbb{P}_2]^3$ and $p_h\in\mathbb{P}_1$, giving
$264\,768$ velocity and $14\,135$ pressure unknowns, $278\,903$ flow degrees
of freedom in total. The apparent viscosity is evaluated at the regularised
shear rate
$\dot{\gamma}_{\varepsilon}=(\varepsilon_{\gamma}^{2}+\dot{\gamma}^{2})^{1/2}$,
with $\dot{\gamma}$ as in \eqref{eq:shear-rate}, and the variational
formulation is augmented by a small pressure regularisation term, by backflow
stabilisation at the inlet and outlets~\citep{EsmailyMoghadam2011}, and by a
weak normal impedance and tangential penalisation at the inlet.

Time integration is first order and semi-implicit. The convective term is
treated by an Oseen--Picard linearisation~\citep{ElmanSilvesterWathen2014}:
the advecting velocity, the Temam divergence correction~\citep{Temam2001},
the Carreau--Yasuda viscosity and the rheological factors of the outlet
resistances are evaluated at the previous time step, so each accepted step
requires a single linearised saddle-point solve. Within a step the coupling
is staggered. The LV-0D driver is advanced first, providing
$p_{\rm ar}^{n+1}$ and $p_{\rm LV}^{n+1}$; the three-dimensional problem is
then solved with $p_{\rm ar}^{n+1}$ imposed at the inlet, the resistive part
of each outlet traction assembled implicitly through
\eqref{eq:coronary-implicit-resistance} and its compartmental part
$p_{\rm im}+p_{tm}^{(i)}$ taken from the previous outlet state; the resulting
outlet fluxes $Q_i^{n+1}$ finally feed the scalar Newton update of each
$p_{tm}^{(i)}$. Only the compartmental pressure is therefore exposed to the
one-step lag, and the stability argument of
\S\ref{sec:coronary-outlet-model} covers the dominant resistive part.

% NOTE: the implementation drains against max(p_im, P_RA), i.e. beta = 1;
% confirm this is the value used for the reported runs before submission.
The simulations of \S\ref{sec:results} use the strong-waterfall limit
$\beta=1$ of \eqref{eq:coronary-starling}. The base time step is
$\Delta t=10^{-3}$~s, halved and retried whenever the three-dimensional solve
fails, for a total simulated time of $2.55$~s (three cardiac cycles of
$0.85$~s). The average computational cost was 12 seconds per accepted time
step on 30 MPI ranks of \textbf{[HARDWARE / CPU DESCRIPTION]}.
}
