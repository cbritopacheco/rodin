...
- Every file under `resources/` must be checked in through Git LFS. When adding
  or replacing a resource, run `git check-attr filter -- <path>` before
  committing and verify `git lfs status` shows the file as an LFS object.
