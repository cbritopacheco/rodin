...
// Test constexpr nature of functions where applicable
TEST_F(CommonTest, ConstexprFunctions)
{
  // Test that abs is constexpr
  constexpr Real abs_val = abs(-5.0);
  EXPECT_DOUBLE_EQ(abs_val, 5.0);
  
  // Test that exp is constexpr  
  constexpr Real exp_val = exp(0.0);
  EXPECT_DOUBLE_EQ(exp_val, 1.0);
  
  // Test that conj is constexpr for complex numbers
  constexpr Complex z(3.0, 4.0);
  constexpr Complex conj_z = conj(z);
