...
TEST_F(AlertTest, WarningConstruction)
{
  ...
  EXPECT_NO_THROW(Warning(warningStream));
}
...
TEST_F(AlertTest, InfoConstruction)
{
  ...
  EXPECT_NO_THROW(Info(infoStream));
}
...
TEST_F(AlertTest, SuccessConstruction)
{
  ...
  EXPECT_NO_THROW(Success(successStream));
}
...
{
  // Test construction without throwing
  EXPECT_NO_THROW(Exception(errorStream));
