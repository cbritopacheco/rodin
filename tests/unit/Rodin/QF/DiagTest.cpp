...
namespace {
  struct R {
    ...
    size_t getSize() const { return o.size(); }
    Real getWeight(size_t i) const { return o[i].getWeight(); }
    Math::SpatialVector<Real> getPoint(size_t i) const {
      Math::SpatialVector<Real> p; p.resize(3);
      for (int k = 0; k < 3; ++k) p[k] = o[i].getBarycentric()[k];
      return p; }
  };
}
TEST(Diag, PyramidRules)
{
  ...
  for (size_t p = 1; p <= 4; ++p)
  {
    ...
    std::printf("pyr d%zu -> %2zu pts (WV %d) err %.1e ok %d (%.0fs)\n",
      p, r.orbits.size(), wv[p-1], rep.worstRelativeError,
      (int)(allWeightsPositive(rule) && allPointsInside(rule, Polytope::Type::Pyramid)), s);
    std::fflush(stdout);
  }
}
