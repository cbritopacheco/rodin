...
int main(int argc, char** argv)
{
  ...
  {
    const auto r = SymmetricRuleGenerator::search(g, p, maxPts, 64, 1e-12, jobs, budget);
    ...
    std::printf("%-4zu %-5s %-5s %-5s %-5zu %-5zu %-6s %.2e%s%s\n", p,
      ...
      wv ? std::to_string(wv).c_str() : ".", xg ? std::to_string(xg).c_str() : ".",
      countingBound(d, p), SymmetricRuleGenerator::invariantDimension(g, p),
