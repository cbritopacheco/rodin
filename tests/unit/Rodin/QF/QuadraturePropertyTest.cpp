...
/**
 * @brief The two families must agree.
 *
 * XiaoGimbutas and WitherdenVincent are generated by different algorithms from
 * different seeds, share no code beyond the moment definitions, and produce
 * different point counts. Two rules of the same strength must nevertheless
 * integrate every polynomial of that degree identically. Agreement between
 * them is evidence neither shares a mistake with the other; a bug common to
 * both would have to be in the moment definitions, which the oracle-free
 * checks above already probe.
 */
TEST(QuadraturePropertyTest, TheTwoFamiliesAgree)
{
...
/// @brief A rule of degree p and one of degree p + 1 must agree on every
/// polynomial of degree p. Consecutive entries of a table are produced by
/// separate searches, so this cross-checks neighbouring rows against each
/// other.
TEST(QuadraturePropertyTest, ConsecutiveDegreesAgree)
{
  ...
  for (const auto g : kSimplices)
  {
    const size_t d = Polytope::Traits(g).getDimension();
    const auto verts = referenceVertices(g);
    for (size_t p = 1; p + 1 <= XiaoGimbutas::getMaxDegree(g); ++p)
    {
      const XiaoGimbutas lo(p, g), hi(p + 1, g);
      for (int trial = 0; trial < 4; ++trial)
      {
        const auto f = randomPolynomial(d, p, rng);
        EXPECT_NEAR(integrateOn(lo, verts, f), integrateOn(hi, verts, f),
          1e-11 * std::max(Real(1), std::abs(integrateOn(hi, verts, f))))
          << name(g) << " degrees " << p << " and " << p + 1;
      }
    }
  }
}
...
/**
 * @brief Permuting the vertices maps the reference element onto itself, so
 * the integral of any polynomial must be unchanged. This exercises the rule
 * under the element's own symmetry group without requiring the rule to be
 * symmetric.
 */
TEST(QuadraturePropertyTest, IntegralIsInvariantUnderVertexPermutation)
{
  std::mt19937 rng(808u);
  for (const auto g : kSimplices)
  {
    const size_t d = Polytope::Traits(g).getDimension();
    auto verts = referenceVertices(g);
    for (size_t p = 1; p <= std::min<size_t>(XiaoGimbutas::getMaxDegree(g), 8); ++p)
    {
      const XiaoGimbutas qf(p, g);
      const auto f = randomPolynomial(d, p, rng);
...
/// @brief The weighted mean of the nodes is the centroid of the element.
/// Follows from exactness on linear functions, and fails loudly if the nodes
/// are systematically displaced.
TEST(QuadraturePropertyTest, WeightedNodeMeanIsTheCentroid)
{
  for (const auto g : kSimplices)
  {
    ...
    {
      const XiaoGimbutas qf(p, g);
      Math::SpatialVector<Real> mean;
      mean.resize(static_cast<Eigen::Index>(d));
      mean.setZero();
      Real total = 0;
      for (size_t q = 0; q < qf.getSize(); ++q)
      {
...
/// @brief Barycentric coordinates of every node sum to one and are
/// non-negative. This is the interiority condition in the coordinates the
/// rules were solved in, rather than in the half-space form.
TEST(QuadraturePropertyTest, NodesHaveValidBarycentricCoordinates)
{
  for (const auto g : kSimplices)
  {
    const size_t d = Polytope::Traits(g).getDimension();
    for (size_t p = 1; p <= XiaoGimbutas::getMaxDegree(g); ++p)
    {
      ...
      for (size_t q = 0; q < qf.getSize(); ++q)
      {
        ...
        for (size_t k = 0; k < d; ++k)
        {
          const Real c = qf.getPoint(q)[static_cast<Eigen::Index>(k)];
...
/**
 * @brief Accuracy on a non-polynomial integrand improves with degree.
 *
 * Exactness on polynomials is a statement about a finite-dimensional space;
 * the reason to want it is that it makes smooth integrands converge. A table
 * whose higher-degree entries were not actually better would satisfy every
 * other test here.
 */
TEST(QuadraturePropertyTest, AccuracyImprovesWithDegreeOnASmoothIntegrand)
{
  for (const auto g : kSimplices)
  {
    const size_t d = Polytope::Traits(g).getDimension();
    ...
    const size_t top = XiaoGimbutas::getMaxDegree(g);
    const Real reference = integrateOn(XiaoGimbutas(top, g), verts, f);
    // Convergence is only observable while the error is above the level at
    // which the reference itself is meaningful. Once a rule is accurate to
    // near machine precision the differences are rounding, and demanding
    // monotone improvement there tests the floating-point noise rather than
    // the rules: on the triangle degree 13 already reaches it.
    constexpr Real floor = 1e-13;
    ...
    for (size_t p = 1; p + 4 <= top; p += 2)
    {
      ...
      if (previous > floor)
      {
        EXPECT_LT(err, previous * 1.05)
          << name(g) << ": degree " << p << " no better than degree " << p - 2;
        ++compared;
      }
      previous = err;
    }
...
/**
 * @brief The inlined tables reproduce the generator.
 *
 * The coefficients are generated, so the risk they carry is not a
 * transcription error but a drift between what the generator now produces and
 * what was inlined when it was last run. Regenerating the low degrees and
 * comparing catches that; the high degrees are too slow to regenerate in a
 * unit test and are covered by the exactness and property checks instead.
 */
TEST(QuadraturePropertyTest, InlinedTablesReproduceTheGenerator)
{
  for (const auto g : kSimplices)
  {
    for (size_t p = 1; p <= 3; ++p)
    {
      const XiaoGimbutas shipped(p, g);
      const auto seed = NodeElimination::productSeed(g, p);
      const auto fresh = NodeElimination::reduce(g, p, seed);
      EXPECT_EQ(shipped.getSize(), fresh.getSize())
        << name(g) << " degree " << p
        << ": regenerating gives a different point count than was inlined";
    }
  }
}
