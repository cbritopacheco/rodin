...
namespace Rodin::Tests::Convergence::H::LinearElasticity
{
  ...
  ManufacturedSolution makeExponentialSolution(
    size_t dim, Real lambda, Real mu)
  {
    return {
      VectorField(dim, VectorCallable([dim](const Point& p)
        {
  ...
  ManufacturedSolution makeDivergenceFreeSolution(size_t dim, Real mu)
  {
    ...
    return {
      VectorField(dim, VectorCallable(displacement)),
  ...
  VectorField makeTraction(
    ...
    const MatrixCallable& exactJacobian,
    Real lambda,
    Real mu)
  {
    const size_t dim = mesh.getSpaceDimension();
    const BoundaryNormal normal(mesh);
    return VectorField(dim, VectorCallable(
      [dim, exactJacobian, lambda, mu, normal](const Point& p)
      {
        ...
        for (size_t i = 0; i < dim; ++i)
        {
          for (size_t j = 0; j < dim; ++j)
          {
            const Real stress = lambda * divergence * (i == j)
              + mu * (jacobian(i, j) + jacobian(j, i));
    ...
    bool mixedTraction = false)
  {
    ...
    {
      P1 vh(mesh, dim);
      return solveProblem<K>(
        mesh, vh, data, lambda, mu, mixedTraction);
    }
    else
    {
  ...
  class LinearElasticityHConvergenceTest
  ...
  {
    testConvergence<1>(GetParam(), {5, 9, 17}, makeExponentialSolution,
      1.5, 0.5, false, 1.65, 2.35, 0.75, 1.25);
  }
  ...
  {
    testConvergence<2>(GetParam(), {3, 5, 9}, makeExponentialSolution,
      1.5, 0.5, false, 2.45, 3.55, 1.55, 2.45);
  }
  ...
  TEST_P(LinearElasticityHConvergenceTest, MixedTractionHasOptimalP1Rates)
  {
    if (GetParam() == Polytope::Type::Tetrahedron)
    {
      ...
      testConvergence<1>(GetParam(), {9, 17, 33}, makeExponentialSolution,
        1.5, 0.5, true, 1.65, 2.35, 0.75, 1.25);
    }
    else
    {
      testConvergence<1>(GetParam(), {5, 9, 17}, makeExponentialSolution,
  ...
  {
    testConvergence<2>(GetParam(), {3, 5, 9}, makeExponentialSolution,
      1.5, 0.5, true, 2.45, 3.55, 1.55, 2.45);
  }
   ...
   */
  TEST_P(LinearElasticityHConvergenceTest,
    NearlyIncompressibleDivergenceFreeHasOptimalP2Rates)
  {
    ...
    testConvergence<2>(GetParam(), {3, 5, 9}, factory,
      1e4, 1, false, 2.25, 3.75, 1.35, 2.65);
  }
  ...
  INSTANTIATE_TEST_SUITE_P(
    ...
    LinearElasticityHConvergenceTest,
    ::testing::Values(
