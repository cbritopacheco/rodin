...
namespace Rodin::Tests::Convergence::H::ReactionDiffusion
{
    ...
    const GradientU& gradientU, const GradientW& gradientW)
  {
    ...
    return {ErrorNorm::compute(mesh, u.getSolution(), exactU, gradientU, 12),
  ...
  void checkRates(Polytope::Type geometry)
  {
    ...
    for (size_t level : hierarchy.getLevels())
    {
      ...
      const auto [uError, wError] =
        solve<K>(mesh, exactU, exactW, sourceU, sourceW, gradientU, gradientW);
  ...
  {};
  TEST_P(ReactionDiffusionTest, P1OptimalRates)
  {
    checkRates<1>(GetParam());
  }
  TEST_P(ReactionDiffusionTest, P2OptimalRates)
  {
