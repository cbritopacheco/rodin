...
`LinearElasticity` uses the same $(n,p)=(2,1)\to(3,2)\to(5,3)$ path for the
analytic vector displacement $u_i=(i+1)\exp(\sum_j x_j)$ with fixed Lamé
parameters $\lambda=1.5$ and $\mu=0.5$. The L2 displacement and H1-seminorm
Jacobian errors are independently integrated on every mesh. The observed
logarithmic ratios measure only this combined path, not fixed-degree h order.
