...
/**
 * @file
 * @brief Curved-geometry map, Poisson and conductivity convergence checks.
 *
...
namespace Rodin::Tests::Convergence::Isoparametric
{
  ...
  /**
   * @brief Variable-coefficient diffusion on the same curved P2 map.
   *
   * With @f$u=e^{\sum x_i}@f$ and @f$\gamma=1+\sum x_i@f$,
   * @f$-\nabla\cdot(\gamma\nabla u)=-d(1+\gamma)u@f$.
   */
  ErrorNorms solveConductivityP2(const LocalMesh& mesh)
  {
    const size_t dim = mesh.getSpaceDimension();
    ...
    const RealFunction forcing([dim, &exact, &gamma](const Point& p) {
      return -Real(dim) * (Real(1) + gamma(p)) * exact(p);
    });
  ...
  void expectOptimalP2Rates(const ErrorHistory& history)
  {
    ...
    for (size_t i = 1; i < history.getSize(); ++i)
    {
      ...
      SCOPED_TRACE(::testing::Message()
        << "L2 " << coarse.getL2() << " -> " << fine.getL2() << ", rate " << rates.getL2()
  ...
  class CurvedGeometryTest : public ::testing::TestWithParam<Polytope::Type>
  {};
   ...
   */
  TEST_P(CurvedGeometryTest, P2GeometryReproducesQuadraticMap)
  {
  ...
  /**
   * @brief Measures the actual P1 interpolation error of the curved map.
   *
   * The comparison is integrated on the original unit-box cells, so it
   * isolates geometry approximation from both PDE and domain-integration
   * errors. A quadratic map has an O(h^2) P1 value-interpolation error.
   */
  TEST_P(CurvedGeometryTest, P1GeometryMapHasSecondOrderError)
  {
    const UniformGrid grid(GetParam());
    const std::initializer_list<size_t> levels = grid.getDimension() == 3
    ...
    for (const size_t pointsPerAxis : levels)
    {
      ...
      {
        const auto referenceCell = reference.getCell(index);
      ...
      history.append(Real(1) / Real(pointsPerAxis - 1), std::sqrt(squaredError));
    }
    ASSERT_EQ(history.getSize(), 3);
    for (size_t i = 1; i < history.getSize(); ++i)
    {
      ...
      SCOPED_TRACE(::testing::Message()
        << "geometry L2 " << coarse << " -> " << fine << ", rate " << rate);
   ...
   */
  TEST_P(CurvedGeometryTest, P2PoissonHasOptimalRatesOnCurvedGeometry)
  {
    const auto geometry = GetParam();
    UniformGrid grid(geometry);
    const std::initializer_list<size_t> levels = grid.getDimension() == 3
  ...
  TEST_P(CurvedGeometryTest, P2ConductivityHasOptimalRatesOnCurvedGeometry)
  {
    const UniformGrid grid(GetParam());
    const std::initializer_list<size_t> levels = grid.getDimension() == 3
      ? std::initializer_list<size_t>{3, 4, 5}
  ...
  INSTANTIATE_TEST_SUITE_P(AllUniformGridGeometries, CurvedGeometryTest,
