...
namespace Rodin::Tests::Benchmarks
{
  ...
  {
    auto mesh = LocalMesh::UniformGrid(Polytope::Type::Triangle, { 16, 16 });
    test(mesh, st);
  }

  BENCHMARK_F(Connectivity, Triangular_32x32)(benchmark::State& st)
  {
    auto mesh = LocalMesh::UniformGrid(Polytope::Type::Triangle, { 32, 32 });
    test(mesh, st);
  }

  BENCHMARK_F(Connectivity, Triangular_64x64)(benchmark::State& st)
  {
    auto mesh = LocalMesh::UniformGrid(Polytope::Type::Triangle, { 64, 64 });
    test(mesh, st);
  }
}
