#ifndef RODIN_CORE_GEOMETRY_LINE2D_IPP
#define RODIN_CORE_GEOMETRY_LINE2D_IPP

#include "Rodin/Core/Geometry/Point2D.h"
...
namespace Rodin::Core::Geometry
{
  ...
  T Line2D<T>::distance(const Line2D<T>& other) const
  {
    if (isVertical() && other.isVertical())
      ...
      return Core::abs<T>(
          *yIntercept() - *other.yIntercept()) / Core::sqrt(*slope() *
          *slope() + T{1});
    else
      return T{0};
  }
  ...
  Line2D<T>::intersect(const Circle<T>& other) const
  {
    ...
    if (discriminant > T{0}) // Two intersections
    {
      return std::make_pair(
          other.center() +
          Point2D<T>(
            (a() * cp + b() * Core::sqrt(discriminant)) / (a() * a() + b() *
              b()),
            (b() * cp - a() * Core::sqrt(discriminant)) / (a() * a() + b() *
              b())
            ),
          other.center() +
          Point2D<T>(
