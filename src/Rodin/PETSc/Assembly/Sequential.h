      ...
      void execute(LinearSystemType& axb, const InputType& input) const override
      {
        ...
        for (auto& dbc : pb.getDBCs())
        {
          ...
          std::visit([&](auto&& dofs)
          {
            ...
            {
              // Identification BC under single-function PETSc path: emit
              // slave-DOF zero pins. Full algebraic identification will be
              // wired through axb.identify(...) when that API lands.
        ...
        Rodin::Variational::Problem<Rodin::PETSc::Math::LinearSystem, U1, U2, U3, Us...>>
  {
    public:
      ...
      void execute(LinearSystemType& axb, const InputType& input) const override
      {
        ...
        std::vector<PetscInt> bcIdx;
        std::vector<PetscScalar> bcVals;
        // Identification rows: pairs (gs, [(gm, c)...]) — assembled below
        ...
        // Apply identification constraint rows directly via MatSetValues:
        //   row gs:  A[gs, gs] = 1, A[gs, gm_k] = -c_k, b[gs] = 0.
        // We zero the slave row first to avoid accumulating with previously
        // assembled entries.
        if (!identRows.empty())
        {
