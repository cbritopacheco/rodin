...
namespace Rodin::Variational
{
  ...
  class DubinerTriangle
  {
    public:
      ...
      /**
       * @brief Gradient of ψ_{P,Q} with respect to the reference coordinates (x,y).
       *
       * Uses ψ = P_p(a) (1-y)^p P_q(b), so that both derivatives carry the
       * factor (1-y)^{p-1} explicitly and no division by (1-y) is performed:
       * the expressions stay valid at the collapsed vertex y = 1.
       *
       * @tparam P First modal index.
       * @tparam Q Second modal index.
       * @param[out] dpsi_dx Derivative w.r.t. x.
       * @param[out] dpsi_dy Derivative w.r.t. y.
          ...
          Real& dpsi_dx, Real& dpsi_dy, Real x, Real y)
      {
        ...
        else
        {
          ...
          dpsi_dy =
            sPm1 * ((a + 1.0) * dPa - static_cast<Real>(P) * Pa) * Pb + 2.0 * Pa * dPb * sP;
        }
      }
  ...
  class DubinerTetrahedron
  {
    public:
      ...
      /**
       * @brief Gradient of ψ_{P,Q,R} with respect to the reference coordinates (x,y,z).
       *
       * Uses ψ = [P_p(a) s1^p] [P_q(b) s2^q] P_r(c) with s1 = 1 - y - z and
       * s2 = 1 - z, so the singular factors cancel analytically and the
       * expressions stay valid on the collapsed edge and vertex.
       *
       * @tparam P First modal index.
       * @tparam Q Second modal index.
       ...
       * @param[out] dpsi_dx Derivative w.r.t. x.
       * @param[out] dpsi_dy Derivative w.r.t. y.
       * @param[out] dpsi_dz Derivative w.r.t. z.
       * @param x First reference coordinate.
       * @param y Second reference coordinate.
       * @param z Third reference coordinate.
       */
      template <size_t P, size_t Q, size_t R>
      static constexpr void getReferenceGradient(Real& dpsi_dx,
                                                 Real& dpsi_dy,
                                                 Real& dpsi_dz,
                                                 Real x, Real y, Real z)
      {
        ...
        // F = P_p(a) s1^p and its derivatives.
        ...
        // G = P_q(b) s2^q and its derivatives.
