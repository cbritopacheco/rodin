              ...
              H1<K, Scalar, Mesh>, TestSpace>>>::ScalarType>
  {
    public:
      ...
      QuadratureRule& setPolytope(const Geometry::Polytope& polytope) final override
      {
        ...
        const H1Element<K, ScalarType> trScalarFE(polytope.getGeometry());
        const H1Element<K, ScalarType> teScalarFE(polytope.getGeometry());
        const size_t scalarCount = trScalarFE.getCount();
        ...
        const auto& trTabS = trScalarFE.getTabulation(*m_qf);
        const auto& teTabS = teScalarFE.getTabulation(*m_qf);
