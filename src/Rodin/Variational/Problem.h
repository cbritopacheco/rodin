...
namespace Rodin::Variational
{
  ...
  class ProblemBase : public FormLanguage::Base
  {
    public:
      ...
      /**
       * @brief Assembles only the left- or right-hand side of the linear system.
       *
       * @param[in] target Selects which side to assemble.
       * @return Reference to this problem instance for method chaining.
       *
       * The default implementation raises an exception. Backends that support
       * targeted assembly (e.g. the PETSc multi-field backend) override this to
       * allow the SNES solver to assemble the Jacobian and residual independently,
       * preventing double assembly during Newton iterations.
       */
