  ...
  std::optional<LineSegment2D<T>> Point2D<T>::connect(const Line2D<T>& line) const
  {
    ...
    if (line(*this) > T{0} || line(*this) < T{0}) // Point is not touching the line
    {
      return LineSegment2D<T>(
          *this,
          Point2D<T>({
            line.b() * (line.b() * Magnum::Math::Vector<Scalar>2<T>::x() - line.a() *
                Magnum::Math::Vector<Scalar>2<T>::y()) + line.a() * line.c(),
            line.a() * (-line.b() * Magnum::Math::Vector<Scalar>2<T>::x() + line.a() *
                Magnum::Math::Vector<Scalar>2<T>::y()) + line.b() * line.c()
