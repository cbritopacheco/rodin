...
/**
 * @file Stepper.h
 * @brief Nonlinear time-stepper for the 0D poroelastic sphere.
 *
 ...
 * from one time step to the next. Same interface as the CCMLC2014 stepper,
 * so it can be driven standalone or as the 0D cavity of a 3D coupling.
 */
#ifndef RODIN_HEART_POROELASTICSPHERE_SOLVER_STEPPER_H
#define RODIN_HEART_POROELASTICSPHERE_SOLVER_STEPPER_H
...
namespace Rodin::Heart::PoroelasticSphere::Solver
{
  /**
   * @brief Time-stepper for the coupled 0D poroelastic sphere.
   *
  ...
  class StepperT
  {
    public:
      ...
      using History = CCMLC2014::Model::HistoryT<State>;
      ...
      class Problem final : public Variational::ProblemBase<DenseLinearSystem>
      {
        public:
           ...
           * @param in Model input parameters (must outlive the problem).
           */
          ...
          Problem& assemble() override
          {
            ...
            EvalData evalData;
            buildEvalData(*m_xCurrent, evalData);
          ...
          /**
           * @brief Evaluates the intermediate data at an unknown vector for
           * the current step data.
           * @param x Unknown vector.
           * @param evalData Output evaluation data.
           */
          void buildEvalData(const DenseVector& x, EvalData& evalData) const
          {
            m_dynamicSystem.buildEvalData(
              x, m_history.n, m_history.nm1, m_time, m_dt, evalData);
          }
       ...
       * @param input Model input parameters (copied).
       */
       ...
       *
       * A zero initial porosity is replaced by the reference porosity
       * @f$ \phi_0 @f$; the active defaults follow CCMLC2014.
       *
      ...
      void initialize(const State& initial)
      {
        ...
        if (initial.phi <= Scalar(0))
          m_state.phi = m_input.phi0;
        ...
        else
        {
          if (initial.kc > Scalar(0))
          {
            ...
            m_state.beta =
              (m_state.gamma > Scalar(0)) ? (initial.tauc / m_state.gamma) : Scalar(0);
          }
          else
          {
            m_state.gamma =
              std::sqrt(std::max<Scalar>(m_input.initActiveStiffness, Scalar(0)));
      ...
      Report step(const Scalar dt)
      {
        ...
        {
          EvalData evalData;
          m_problem.buildEvalData(m_x, evalData);
       ...
       */
      void restore(const State& state, const History& history,
        const DenseVector& unknowns, const Report& report)
      {
        m_state = state;
        m_history = history;
        m_x = unknowns;
    ...
    private:
      ...
      ::Rodin::Solver::NewtonSolver<::Rodin::Solver::PartialPivLU<DenseLinearSystem>>
        m_newton;
  };
}
