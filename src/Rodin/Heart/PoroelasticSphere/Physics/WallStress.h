...
namespace Rodin::Heart::PoroelasticSphere::Physics
{
  ...
  class WallStressEvaluator
  {
    public:
      ...
      void evaluate(EvalData& data) const
      {
        ...
        for (size_t k = 0; k < m_x.size(); ++k)
        {
          ...
          const Scalar Ptt =
            pas.Ptt + eta * q.lt * q.lt * q.ltDot + Scalar(0.5) * sigma * q.lt;
    ...
    private:
       ...
       */
      static void gaussLegendreUnit(size_t n, std::vector<Real>& x, std::vector<Real>& w)
      {
        assert(n >= 1);
        x.assign(n, Real(0));
        w.assign(n, Real(0));
        ...
        {
          Real z = std::cos(
            std::numbers::pi_v<Real> * (Real(k) + Real(0.75)) / (Real(n) + Real(0.5)));
          Real dp = Real(1);
          for (size_t it = 0; it < 100; ++it)
          {
            ...
            {
              const Real p =
                ((Real(2) * j - Real(1)) * z * p1 - (Real(j) - Real(1)) * p0) / Real(j);
              p0 = p1;
              p1 = p;
            }
