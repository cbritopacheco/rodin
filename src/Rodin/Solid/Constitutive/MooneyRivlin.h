  ...
  class MooneyRivlin final : public HyperElasticLaw<MooneyRivlin>
  {
    public:
          ...
          const KinematicState& state) const
      {
        ...
        const Math::Matrix<Real> dI1bar_dF =
          cache.Jm2d * (2.0 * F - (2.0 / dd) * cache.I1 * FinvT);

        // dI2bar/dF = Jm4d (2 I1 F - 2 F C - (4/d) I2 F^{-T})
        const Math::Matrix<Real> dI2bar_dF =
          cache.Jm4d * (2.0 * cache.I1 * F - 2.0 * F * C - (4.0 / dd) * cache.I2 * FinvT);

        // dJvol/dF = kappa (J - 1) J F^{-T}
        const Math::Matrix<Real> dJvol_dF =
          ...
          const Math::Matrix<Real>& dF) const
      {
        ...
        const Real F_dF = (F.array() * dF.array()).sum();

        // Derivative of F^{-T} in direction dF: d(F^{-T}) = -F^{-T} dF^T F^{-T}
        const Math::Matrix<Real> dFinvT = -FinvT * dF.transpose() * FinvT;

        // ---- I1bar contribution ----
        // d(Jm2d)/ddF term + Jm2d d(2F - 2/d I1 F^{-T})/ddF
        const Real dJm2d = -(2.0 / dd) * cache.Jm2d * FinvT_dF;
        const Real dI1 = 2.0 * F_dF;
        const Math::Matrix<Real> d_dI1bar_dF =
          dJm2d * (2.0 * F - (2.0 / dd) * cache.I1 * FinvT)
          + cache.Jm2d * (2.0 * dF - (2.0 / dd) * (dI1 * FinvT + cache.I1 * dFinvT));

        // ---- I2bar contribution ----
        // I2 = 0.5(I1^2 - tr(C^2)) so dI2 = I1 dI1 - tr(C dC) where dC = dF^T F + F^T dF
        const Math::Matrix<Real> dC = dF.transpose() * F + F.transpose() * dF;
        const Real dI2 = cache.I1 * dI1 - (C.array() * dC.array()).sum();

        const Real dJm4d = -(4.0 / dd) * cache.Jm4d * FinvT_dF;
        const Math::Matrix<Real> d_dI2bar_dF =
          dJm4d * (2.0 * cache.I1 * F - 2.0 * F * C - (4.0 / dd) * cache.I2 * FinvT)
          + cache.Jm4d * (2.0 * dI1 * F + 2.0 * cache.I1 * dF
                        - 2.0 * dF * C - 2.0 * F * dC
                        - (4.0 / dd) * (dI2 * FinvT + cache.I2 * dFinvT));

        // ---- Volumetric contribution ----
        const Real dJ = cache.J * FinvT_dF;
        const Math::Matrix<Real> d_dJvol_dF =
